...
func _process(delta):
	...
	# lock our X position
	if get_pos().x != x_pos:
		set_pos(Vector2(x_pos, get_pos().y))
...
func _integrate_forces(s):
	...
	if fire_timer >= fire_rate and burst_counter < burst_count:
		...
		if burst_timer >= burst_rate:
			...
			var d = sign(get_node('/root/Game/Toon').get_pos().x - get_pos().x)
			var y = rand_range(-4,4)
			
			bullet.set_linear_velocity( Vector2(get_node('/root/globals').SLOWBALL_SPEED*d, y) )
			burst_timer = 0
			burst_counter += 1
	...
	set_linear_velocity(Vector2(0,SPEED*direction))
